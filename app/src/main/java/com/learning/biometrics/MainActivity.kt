    ...
    private fun checkAndStartBiometricAuthentication() {
        ...
        when (biometricManager.canAuthenticate(BIOMETRIC_STRONG or DEVICE_CREDENTIAL)) {
            BiometricManager.BIOMETRIC_ERROR_HW_UNAVAILABLE -> {
                ...
                binding.tvMessage.text = getString(R.string.bioMetricUnAvailable)
            }
            BiometricManager.BIOMETRIC_ERROR_NONE_ENROLLED -> {
                ...
                binding.tvMessage.text = getString(R.string.bioMetricNotEnrolled)
            }
            BiometricManager.BIOMETRIC_ERROR_NO_HARDWARE -> {
                ...
                binding.tvMessage.text = getString(R.string.bioMetricNotAvailable)
            }
            BiometricManager.BIOMETRIC_ERROR_SECURITY_UPDATE_REQUIRED -> {
                Log.d(TAG, "checkBiometricAuthentication: Security Update required")
                binding.tvMessage.text = getString(R.string.SecurityUpdateRequired)
            }
            BiometricManager.BIOMETRIC_ERROR_UNSUPPORTED -> {
                Log.d(TAG, "checkBiometricAuthentication: Error Unsupported")
                binding.tvMessage.text = getString(R.string.UnsupportedError)
            }
            BiometricManager.BIOMETRIC_STATUS_UNKNOWN -> {
                Log.d(TAG, "checkBiometricAuthentication: Error Unknown Status")
                binding.tvMessage.text = getString(R.string.UnknownStatusError)
            }
            BiometricManager.BIOMETRIC_SUCCESS -> {
                Log.d(TAG, "checkBiometricAuthentication: Biometric Success")
                binding.tvMessage.text = getString(R.string.BioMetricSuccess)
